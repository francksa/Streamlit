...
MSA_FILTER_VALUE = 1
WEIGHT_COL = "wts"
AWARE_COL = "RS3_1"  # The anchor for Brand Awareness
...
# --- FIXED LOGIC: Base = Awareness (RS3_1) ---
def weighted_brand_salience(rs8_series: pd.Series, aware_series: pd.Series, w: np.ndarray) -> float:
    """
    Dashboard-aligned: weighted % selecting TFM among BRAND AWARE respondents.
    """
    aware_mask = (aware_series == 1).to_numpy()
    if aware_mask.sum() == 0:
        return 0.0
    # Success is selecting TFM (1). If they are aware but didn't select it, it's 0.
...
def build_X_and_eligibility(raw: pd.DataFrame, aware_series: pd.Series, cep_idx: list[int]) -> tuple[np.ndarray, np.ndarray]:
    """
    Builds X (associations) and elig (Awareness mask).
    """
    n = len(raw)
    k = len(cep_idx)
    ...
    for j, cep in enumerate(cep_idx):
        ...
        elig[:, j] = aware_mask # Eligibility is now based on Awareness
...
def main():
    ...
    st.markdown("""
    **Aligned to Brand Awareness Logic:**
    - **TFM CEP Salience** is calculated as the weighted % of **Brand Aware** respondents who associate TFM with the CEP.
    """)
    ...
    # Calculate Metrics using Awareness Base
    ...
    }).sort_values("Category salience (wtd
