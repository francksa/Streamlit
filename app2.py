...
MSA_FILTER_VALUE = 1
WEIGHT_COL = "wts"
AWARE_COL = "RS3_1NET"  # The anchor for Brand Awareness logic
...
def weighted_brand_salience(rs8_series: pd.Series, aware_series: pd.Series, wts: np.ndarray) -> float:
    """Calculates salience % ONLY among respondents aware of TFM."""
    aware_mask = (aware_series == 1).to_numpy()
    if aware_mask.sum() == 0:
        return 0.0
    # Success = 1, Failure = 2. We treat anything else as 0.
...
def simulate_unique_reach(X, elig, wts, salience_current_w, uplifts_pts, n_sims, cap):
    """Monte Carlo deduplication of households."""
    ...
    for t in range(n_sims):
        X_sim = X.copy()
        U = rng.random((n, k))
        # respondents only flip if they are Aware (elig) and don't currently associate
...
def main():
    ...
    # Discover CEPs and Calculate Baselines
    cep_idx = [int(m.group(1)) for c in raw.columns if (m := re.match(r"RS1_(\d+)NET$", str(c)))]
    
    # Calculate Salience among AWARE respondents (Matches Purple Graphs)
    ...
    uplifts = np.zeros(k)
    order = np.argsort(-prevalence_w) # Order by most common category behaviors
    for j in order:
    ...
    points = base.mark_circle(opacity=0.8, stroke="black", strokeWidth=1).encode(
        ...
        color="CEP Type",
        tooltip=["CEP", "Current Brand Salience %", "Scenario Brand Salience %", "Accessible TAM (HHs)"]
    )
